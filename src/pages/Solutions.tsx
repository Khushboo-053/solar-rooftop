        ...
        <section id="residential" className="bg-card rounded-[2rem] overflow-hidden grid grid-cols-1 lg:grid-cols-2 mb-8 shadow-sm scroll-mt-32">
          ...
          <div className="p-10 lg:p-14 flex flex-col justify-center">
            ...
            <ul className="space-y-3 mb-8">
              {[
                { text: "Save ₹1,000 - ₹10,000 per month on bills", color: "text-green-600" },
                { text: "10+ Years Manufacturer Performance Warranty", color: "text-purple-600" },
                { text: "PM Surya Ghar Subsidy Support", color: "text-tertiary" },
              ].map((p) => (
                <li key={p.text} className="flex items-center gap-3">
                  <span className={`material-symbols-outlined ${p.color} text-xl`}>check</span>
                  <span className="text-on-surface">{p.text}</span>
                </li>
              ))}
            </ul>
